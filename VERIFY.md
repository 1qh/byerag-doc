...
- [x] **Version conflict** (same filename, same scope, different content): blocking modal `a different file with this name already exists. Replace it? Keep both? Cancel?`
  ...
  - [x] **Cancel** → no row, no blob; staged tmp file deleted.
...
- [x] From inside sandbox: `curl <convex-internal>/api/anthropic/v1/messages` with valid bearer succeeds.
...
- [x] 51st approval (stretch path) succeeds with banner.
