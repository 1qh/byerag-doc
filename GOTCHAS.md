...
- **Admin `/dashboard` + `/policy` SSR-500 "Panic in async function" under Next 16 Turbopack** — both pages return 500 with body `{"err":{"name":"Error","source":"server","message":"Panic in async function"}}`; the panic is the Turbopack swc layer, not page code. Symptom: `/` (admin redirect target) and `/policy` both unreachable in the browser despite `/docs`, `/quarantine`, `/users`, `/audit`, `/deleted` rendering fine. Root cause + canonical fix: TBD — surfaced 2026-06-03 during admin app sweep; under investigation.
