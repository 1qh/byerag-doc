...
- A "compare these two docs" / "which should I follow" request is a conflict-resolution request, not a diff request. The user-app system prompt carries an explicit MANDATORY protocol: use `docs conflict` (not bare `docs diff`), always probe `docs similar --scope shared` for a canonical authority even when the user did not mention shared docs, and either cite the canonical value or state plainly that no shared authority exists + recommend admin escalation. A bare difference table with no guidance is a wrong answer. Left only to the generic supportiveness wording the agent defaulted to `docs diff` and stopped; the explicit compare-path block in the prompt is the enforcement surface (prompt = behavioral lever per `system-prompts.md`).
- The mandatory shared-corpus probe is NOT compare-only. ANY substantive document question — including single-doc "summarize" / "highlights" / "what does this say" — must, after answering from the target doc, run `docs similar --scope shared` and surface a genuinely-related shared policy/obligation/risk with citations (or add nothing if none is genuinely related — no manufactured links). A faithful summary of only the uploaded file with no shared-corpus connection when a relevant one exists is incomplete. Scoping the mandate to compares alone was insufficient; the user-app prompt now enforces it for every doc question. This is the `AGENT-DOCTRINE.md` supportiveness "cross-reference proactively" bar made enforceable, not optional.
