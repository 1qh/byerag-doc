...
- gVisor runtime: deferred to prod Linux per `docs/adr/docker-gvisor-sandbox.md`. Local Colima dev uses `runc` w/ `--cap-drop ALL --security-opt no-new-privileges --read-only --tmpfs` for hardening parity. `sandboxClient.ts HostConfig.Runtime = env.SANDBOX_RUNTIME` (default `runc`); operator sets to `runsc` on prod Linux server.
...
- `apps/backend/convex/dashboard.ts` — `costCycleHistory` (last N cycles 5th-to-5th, current `isCurrent`); `topStrip` (users, cycle cost, docs-in-corpus, pendingSuggestions, policyPendingDocs, quarantineDocs, activeChats); `costCyclePivot`; training queries backing the Training page (KPI cards, overdue via `assignment_due_days`, tests rows, paginated/searchable users roster).
- `apps/admin/src/app/(standalone)/dashboard/page.tsx` — stats-only: hero cost + Δ vs last cycle, attention tiles, compact cost history (click bar → pivot), $-sorted Top spenders. No assignment surface.
- `apps/admin/src/app/(standalone)/training/page.tsx` — Training page: KPI cards, Tests table with per-topic ⋯ actions, agent on/off + capability callout + heartbeat + activity feed, paginated/searchable Users roster (department, passed/assigned, overdue) with inline per-user test-status expand.
- `apps/{admin,user}/src/app/(main)/docs/page.tsx` — DocUpload widget + listShared/listMine doc lists.
- `apps/backend/convex/training.ts:resolvePairAction` — atomic conflict-pair/cap-swap resolver (accept-swap / keep-old / keep-both / reject-both); inferBatchSubstantive query — has-retire→substantive, else cosmetic.
